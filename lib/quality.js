...
export function checkNote({ title, body, tags, images = [], cards = [] }) {
  ...
  if (charCount === 0) {
    add("fail", "文案", "正文为空");
  } else if (charCount < 300 || charCount > 600) {
    const near = charCount >= 250 && charCount <= 700;
    add(near ? "warn" : "fail", "文案", `正文字数 ${charCount} 字（硬规则 300-600 字，MCP上限1000需留余量）`);
  } else {
    add("pass", "文案", `正文字数 ${charCount} 字 ✓（300-600）`);
  }
