...
export class ImageGen {
  constructor(config = {}) {
    ...
    // 关闭生图兜底（config.local.json：imageGeneration.disableGen = true）
    this.disableGen = config.disableGen === true;
  ...
  // 下载远程图片 → Buffer（带 UA + 自动 Referer 防盗链：Referer 设为图片同源站点）
  ...
  // 按图片内容 magic bytes 推断扩展名（PNG/JPEG/WebP），避免「PNG 存成 .jpg」
  static imageExt(buffer) {
  ...
  // 从 markdown/Jina 正文提取图片 URL（过滤头像/logo/二维码/图标类小图）
  extractFromMarkdown(text) {
    const urls = [...String(text || "").matchAll(/!\[[^\]]*\]\((https?:\/\/[^)\s]+)\)/g)].map((m) => m[1]);
    return urls.filter((u) => !this.isJunkImage(u)).slice(0, 4);
  }

  // 垃圾图判定：logo/头像/二维码/图标/横幅/表情
  isJunkImage(url) {
    return /avatar|logo|icon|favicon|qrcode|banner|sponsor|emoji|badge|weixin|wechat|share_|follow|\.svg($|\?)/i.test(url);
  }

  // 从 HTML 提取正文图片（og:image 优先 + 所有 <img>，过滤垃圾图）
  extractFromHtml(html) {
    const og = html.match(/<meta[^>]+property=["']og:image["'][^>]+content=["']([^"']+)["']/i)
      || html.match(/<meta[^>]+content=["']([^"']+)["'][^>]+property=["']og:image["']/i);
    if (og && !this.isJunkImage(og[1])) return og[1];
    // data-src（懒加载）+ src 全量提取，过滤垃圾
    const imgs = [
      ...html.matchAll(/<img[^>]+(?:data-src|src)=["'](https?:\/\/[^"']+\.(?:jpe?g|png|webp))["']/gi),
    ].map((m) => m[1]).filter((u) => !this.isJunkImage(u));
    return imgs[0] || null;
  }

  // Bing 图片搜索（cn.bing.com，无需 API Key）：HTML 里的 m 属性含原图 murl
  // 返回真实网络图片 URL 列表（过滤垃圾图；无尺寸元数据，靠下载容错）
  async searchWebImages(query, { count = 6 } = {}) {
    ...
    try {
      const res = await fetch(
        `https://cn.bing.com/images/search?q=${encodeURIComponent(q)}&form=HDRSC2&first=1&tsc=ImageHoverTitle`,
        {
          headers: {
            "User-Agent":
      ...
      // m 属性是 HTML 实体编码的 JSON：&quot;murl&quot;:&quot;https://...
      const urls = [
        ...html.matchAll(/murl&quot;:&quot;(https?:\/\/[^&"]+?)&quot;/g),
      ]
        .map((m) => m[1])
        .filter((u) => /\.(jpe?g|png|webp)(\?|$)/i.test(u) && !this.isJunkImage(u));
      // 去重
      const seen = new Set();
      const unique = urls.filter((u) => !seen.has(u) && seen.add(u));
      return unique.slice(0, count).map((url) => ({ url }));
    } catch {
      return [];
    } finally {
      clearTimeout(timer);
  ...
  // 从草稿提炼图片搜索词（标题核心词 + 主题词，去掉语气词）
  buildImageQuery(draft) {
    const title = String(draft?.title || "").replace(/[，。！？、,.!?：:；;""''（）()\s]/g, " ");
