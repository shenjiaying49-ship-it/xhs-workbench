...
export class McpClient {
  ...
  async publishContent({ title, content, imagePaths, tags, visibility, isOriginal }) {
    ...
    try {
      return await this.callTool("publish_content", args);
    } catch (error) {
      // 断网/导航/上传超时类错误：发布动作未触达（连页面都没打开或图没传完），
      // 确认不会重复发布 → 等待网络恢复后安全重试一次
      const msg = String(error.message || error);
      if (/INTERNET_DISCONNECTED|导航到发布页面失败|上传图片失败|上传超时/i.test(msg)) {
        await new Promise((r) => setTimeout(r, 10000));
        return this.callTool("publish_content", args);
      }
      throw error;
    }
  }
