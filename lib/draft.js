...
// 红线：不引用外部案例、不编造数据/效果/身份；标题 ≤20 字（MCP 上限）；正文 300-1000 字
...
【正文硬规则（违反任何一条都无法发布）】
- 600-950 字（不含空格换行；目标 650-900 字最稳，硬上限 1000 字）——内容展开充分，把每个观点写透，不要提前收尾
- 全文零 emoji
...
- 不引用第三方机构、报告、专家观点做背书
${isMaterial ? `- 素材原文仅作为创作依据（上限 500 字），你的输出不受字数限制——正文按硬规则写 650-900 字并充分展开\n` : ""}
...
export class DraftGenerator {
  ...
  async generate({ topic, analysis, persona, recentTitles, exemplar, checkFn }) {
    ...
    if (fails.length || bodyLen < 550) {
      const problems = [...fails.map((f) => f.message)];
      if (bodyLen < 550) problems.push(`正文仅 ${bodyLen} 字，深度不够——必须扩写到 650-900 字：展开观点背后的逻辑、补充对读者的具体影响、把利他清单写实（可执行动作/判断标准），不要注水不要重复`);
...
重新输出修复后的完整内容（同样用【标题】【正文】【标签】分隔格式）：`;
      try {
        ...
        if (repairedFails < fails.length || (bodyLen < 550 && repairedLen >= bodyLen && repairedFails <= fails.length)) {
          draft = sanitized;
