...
function buildPrompt({ topic, analysis, persona, recentTitles }) {
...
推荐标题风格：${(analysis.titleDirections || []).join("、")}`
  ...
  return `你是「${persona.name}」的小红书内容主笔。账号简介：${persona.desc}
...
【任务】基于这个热点，产出一篇可以直接微调发布的小红书图文草稿。
...
【正文硬规则（违反任何一条都无法发布）】
- 300-600 字（不含空格换行；目标 380-500 字最稳，低于 300 字会被系统拦截）
- 全文零 emoji
- 分点最多 3 个，编号格式为纯数字单独一行（如"1."）
- 每个段落 1-3 句话
- 不以问号结尾
- 不使用分隔线（---、=== 等）
- 不堆砌加粗短句
- 结尾固定动作：预告下一篇内容 + 给一个关注理由（一两句话，自然融入）
- 第一人称"大厂裸辞做 AI 观察"的人设视角，说人话，有判断有态度
- 事实只能来自上面给的热点信息，不许编造数据、价格、效果承诺、外部案例
- 严禁出现输入里没有的具体数字和百分比（如"30% 的岗位""涨价 50%"），预测和判断只能用"可能""不少""越来越多"等模糊表述
	- 不引用第三方机构、报告、专家观点做背书
	${isMaterial ? `- 素材原文仅作为创作灵感（上限 500 字），你的输出不受字数限制——正文按下方硬规则写 380-500 字并充分展开` : ""}
...
export class DraftGenerator {
  ...
  async generate({ topic, analysis, persona, recentTitles, checkFn }) {
    if (!this.available) {
    ...
    const userPrompt = buildPrompt({ topic, analysis, persona, recentTitles });

    let draft = extractDraft(await this.chat(systemPrompt, userPrompt));
    draft = this.sanitize(draft, topic);
    ...
    if (fails.length || bodyLen < 300) {
      const problems = [...fails.map((f) => f.message)];
      if (bodyLen < 300) problems.push(`正文仅 ${bodyLen} 字，必须扩写到 380-500 字（补充具体细节、使用场景、判断，不要注水）`);
...
重新输出修复后的完整内容（同样用【标题】【正文】【标签】分隔格式）：`;
      try {
        ...
        const sanitized = this.sanitize(repaired, topic);
        ...
        if (repairedFails < fails.length || (bodyLen < 300 && repairedLen >= bodyLen && repairedFails <= fails.length)) {
          draft = sanitized;
